...
// TODO: copy this component as a page into pages/[name]/contact.tsx
// Move [name].tsx into pages/[name]/index.tsx
export const ContactPanel = ({
  ...
  function selectTopic(event) {
    ...
    // TODO: replace this with subscription to errors obj
    let i = document.querySelector("#contactSubject") as HTMLInputElement;
    if (i.disabled) {
      setError("");

      // TODO: replace this with tailwindcss style, sub to "topic" var
  ...
  // TODO: replace this with validate on focus
  ...
  return (
    <form
      ...
      className={`${className} fixed top-0 left-0 ${
        separateTab ? "bg-white w-full h-full min-h-184 pt-0 px-3 pb-16" : ""
      }`}>
      ...
      <div id="messageWrap" className="mt-6 mx-6 mb-36;" onClick={messageClick}>
        <input
          className="border-box rounded-none border border-solid border-black w-full outline-0 placeholder:text-slate-400 disabled:text-slate-300 disabled:placeholder:text-slate-300 px-3 py-3.5"
          type="text"
          placeholder="Subject"
          id="contactSubject"
          disabled
      ...
      {separateTab ? (
        ...
        <div
          className="inline-block mt-16 mx-auto mb-0 before:content-[''] before:bg-[url('/extarrow.png')] before:h-3 before:w-16 before:mr-4 before:bg-center before:bg-auto before:bg-no-repeat"
          onClick={onClick}>
          Back to artist info
        </div>
      )}
